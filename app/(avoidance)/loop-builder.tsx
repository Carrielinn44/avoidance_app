...
export default function LoopBuilder() {
  type TemplateKey = 'A' | 'B' | 'C' | 'CUSTOM';
  const [choice, setChoice] = useState<TemplateKey>('A');
  const [custom, setCustom] = useState(['', '', '']);

  const steps = choice === 'CUSTOM' ? custom : templates[choice as keyof typeof templates];
	...
	<View style={lbStyles.container}>
	  <View style={lbStyles.card}>
		...
		<View style={lbStyles.row}>
		  {['A', 'B', 'C', 'CUSTOM'].map(k => (
			<TouchableOpacity key={k} style={[lbStyles.pill, choice === k && lbStyles.pillActive]} onPress={() => setChoice(k as TemplateKey)}>
